...
def calculate_alpha(epoch, max_epochs):
    # 计算 epoch 与最大 epoch 数目的比值
    ratio = (epoch / max_epochs) if ((epoch / max_epochs) < 0.4) else 0.4
    # 根据比值计算 alpha 值的增加量
    alpha_increase = ratio * 0.49
    # 计算最终的 alpha 值
    return alpha_increase


def distance_map_array_gen(array):
    distance_map_data = np.zeros_like(array, dtype=float)

    # 逐批次和通道计算距离图
    for batch_idx in range(array.shape[0]):
        for channel_idx in range(array.shape[1]):
            mask_each_volume = array[batch_idx, channel_idx, :, :, :]

            if mask_each_volume.sum() < 1:
                continue

            # 将矩阵转换为 UInt16 数据类型
            array_uint16 = mask_each_volume.astype(np.uint16)
            array2img = sitk.GetImageFromArray(array_uint16)
            filter = sitk.SignedMaurerDistanceMapImageFilter()
            filter.SetUseImageSpacing(False)
            filter.SetSquaredDistance(False)
            img_dist = filter.Execute(array2img)

            # Get Array
            distance_map_data[batch_idx, channel_idx, :, :, :] = sitk.GetArrayFromImage(img_dist)

            # 将 UInt16 数据类型的矩阵转换回原始数据类型
    # distance_map_data = distance_map_data.astype(array.dtype)

    # to tensor
    distance_array = torch.tensor(distance_map_data)
