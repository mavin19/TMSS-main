...
class ToTensor:
    ...
    def __call__(self, sample):
        if self.mode == 'train':
            ...
            # img = torch.from_numpy(img).float().permute(2, 3, 0, 1)
            # mask = torch.from_numpy(mask).float().permute(2, 3, 0, 1)
            # img = F.interpolate(img, 80).permute(1, 2, 3, 0)
            # mask = F.interpolate(mask, 80).permute(1, 2, 3, 0)
...
class Mirroring:
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            ...
            random_axes = random.sample(range(1, 4), n_axes)
...
class RandomRotation:
    ...
    def rotate_3d_along_axis(img, angle, axis, order):
        ...
        if axis == 1:
            rot_img = np.transpose(img, axes=(0, 2, 3, 1)).squeeze(0)
            rot_img = rotate(rot_img, angle, order=order, preserve_range=True)
            rot_img = np.transpose(rot_img, axes=(2, 0, 1)).unsqueeze(0)

        if axis == 2:
            rot_img = np.transpose(img, axes=(0, 3, 1, 2)).squeeze(0)
            rot_img = rotate(rot_img, angle, order=order, preserve_range=True)
            rot_img = np.transpose(rot_img, axes=(1, 2, 0)).unsqueeze(0)
    ...
    def __call__(self, sample):
        if self.mode == 'train':
            ...
            if not all(i == 0 for i in (d_x, d_y, d_z)):
                ...
                img = np.pad(img, ((0, positive[0]), (0, positive[1]), (0, positive[2]), (0, 0)), 'constant',
                             constant_values=(0, 0))
                mask = np.pad(mask, ((0, positive[0]), (0, positive[1]), (0, positive[2]), (0, 0)), 'constant',
                              constant_values=(0, 0))
                ...
                assert img.shape[:-1] == mask.shape[
                                         :-1], f'Shape mismatch for the image {img.shape[:-1]} and mask {mask.shape[:-1]}'
...
class ExtractPatch:
    ...
    def __call__(self, sample):
        ...
        if random.random() < self.p_tumor:
            ...
            patch_org_x = np.clip(patch_org_x, 0, img.shape[1] - ps_x)

            patch_org_y = random.randint(tumor_center_y - ps_y, tumor_center_y)
            patch_org_y = np.clip(patch_org_y, 0, img.shape[2] - ps_y)

            patch_org_z = random.randint(tumor_center_z - ps_z, tumor_center_z)
            patch_org_z = np.clip(patch_org_z, 0, img.shape[3] - ps_z)
        else:
            patch_org_x = random.randint(0, img.shape[1] - ps_x)
            patch_org_y = random.randint(0, img.shape[2] - ps_y)
            patch_org_z = random.randint(0, img.shape[3] - ps_z)
...
class ElasticDeformation:
    ...
    def __call__(self, sample):
        ...
        ct_img = images
        if random.random() < self.p:
            new_ct, new_mask = elasticdeform.deform_random_grid([ct_img, mask],
                                                                sigma=random.randint(5, 10),
                                                                points=random.randint(1, 3), axis=(1, 2, 3))
