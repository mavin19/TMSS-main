...
def get_paths_to_patient_files(path_to_imgs, PatientID, append_mask=True):
       ...
       # patients = [p for p in PatientID if os.path.isdir(path_to_imgs / p)]
        paths = []
        for p in PatientID:
            path_to_ct = path_to_imgs / 'imagesTr' / (p + '_before.nii')
            path_to_pt = path_to_imgs / 'imagesTr' / (p + '_after.nii')

            if append_mask:
                path_to_mask = path_to_imgs / 'labelsTr_nii' / (p + '.nii')
...
class HecktorDataset(Dataset):
    ...
    def make_data(self, path):
        ...
        #clinical_data["time"] = scale(clinical_data["time"])
        ...
        #clinical_data = pd.get_dummies(clinical_data, columns=["Gender"])
