...
class UNETR_PP(nn.Module):
    ...
    def __init__(
        ...
        self.decoder2 = UnetrUpBlock(
            ...
            out_channels=feature_size,
            kernel_size=3,
            upsample_kernel_size=(6, 4, 4),        # (1,4,4)
            norm_name=norm_name,
            out_size=192 * 160 * 160,
            conv_decoder=True,
        )
    ...
    def forward(self, x_in):
        ...
        if self.do_ds:
            ...
            logits = self.out1(out).permute(0, 1, 3, 4, 2)

        x_output = torch.mean(x_output, dim=1)
        #print("x_output  ", x_output.shape)
        risk_out = self.mtlr(x_output)
        #print("logits  ", logits.shape)
        #print("risk_out  ", risk_out.shape)

        return logits, risk_out
